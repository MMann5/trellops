...
export function Workspace() {
  ...
  const onAddEmptyBoard = () => {
    dispatch(setBoards([...boards, getEmptyBoard(boardName, boardBackground)]));
    setBoardName(''); 
  ...
  return (
    <div className={(isPopShown)?'work-space opacity':'work-space'} >
      <BoardsNavBar />
      {/* <h2>Workspace</h2> */}
        ...
        <div className='work-space-boards'>
          {boards.map((board, idx) => (
            <Link to={`/board/${board._id}`} key={idx}>
              <div className='board-preview flex justify-center' style={{ backgroundColor: board.style?.bgColor, backgroundImage: `url(${board.style?.bgColor})`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
                {board.title}
                {/* <button
          ...
          <div className='board-preview new flex justify-center' onClick={()=>onShowPopup()}>
            Create new board
          </div>
        </div>
      </div>
      {isPopShown && <WorkspacePopup onAddEmptyBoard={onAddEmptyBoard} setBoardName={setBoardName} boardName={boardName} onClosePopup={onClosePopup} setboardBackground={setboardBackground} boardBackground={boardBackground} /> }
    </div>
  );
}
